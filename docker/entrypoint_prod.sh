...
# queries do not consume a gunicorn worker for unbounded lengths of time. 
    ...
    exec gunicorn treeherder.config.wsgi:application --timeout 30 --bind 0.0.0.0
...
# TODO: Merge these two listeners into one since they use so little CPU each (bug 1530965).
    ...
    exec ./manage.py pulse_listener_pushes
elif [ "$1" == "pulse_listener_tasks" ]; then
    exec ./manage.py pulse_listener_tasks
elif [ "$1" == "pulse_listener_tasks_classification" ]; then
    exec ./manage.py pulse_listener_tasks_classification
...
# Processes pushes/jobs from Pulse that were collected by `pulse_listener_{pushes,tasks,tasks_classification}`.
    ...
    export REMAP_SIGTERM=SIGQUIT
    exec celery -A treeherder worker --without-gossip --without-mingle --without-heartbeat -Q store_pulse_pushes,store_pulse_tasks,store_pulse_tasks_classification --concurrency=3

# Handles the log parsing tasks scheduled by `worker_store_pulse_data` as part of job ingestion.
elif [ "$1" == "worker_log_parser" ]; then
    export REMAP_SIGTERM=SIGQUIT
    exec celery -A treeherder worker --without-gossip --without-mingle --without-heartbeat -Q log_parser --concurrency=7
elif [ "$1" == "worker_log_parser_fail_raw_sheriffed" ]; then
    export REMAP_SIGTERM=SIGQUIT
    exec celery -A treeherder worker --without-gossip --without-mingle --without-heartbeat -Q log_parser_fail_raw_sheriffed --concurrency=1
elif [ "$1" == "worker_log_parser_fail_raw_unsheriffed" ]; then
    export REMAP_SIGTERM=SIGQUIT
    exec celery -A treeherder worker --without-gossip --without-mingle --without-heartbeat -Q log_parser_fail_raw_unsheriffed --concurrency=1
elif [ "$1" == "worker_log_parser_fail_json_sheriffed" ]; then
    export REMAP_SIGTERM=SIGQUIT
    exec celery -A treeherder worker --without-gossip --without-mingle --without-heartbeat -Q log_parser_fail_json_sheriffed --concurrency=7
# elif [ "$1" == "worker_log_parser_fail_json_unsheriffed" ]; then
#     export REMAP_SIGTERM=SIGQUIT
#     celery -A treeherder worker --without-gossip --without-mingle --without-heartbeat -Q log_parser_fail_json_unsheriffed --concurrency=7
elif [ "$1" == "worker_perf_ingest" ]; then
    export REMAP_SIGTERM=SIGQUIT
    exec celery -A treeherder worker --without-gossip --without-mingle --without-heartbeat -Q perf_ingest --concurrency=7

# Tasks that don't need a dedicated worker.
elif [ "$1" == "worker_misc" ]; then
    export REMAP_SIGTERM=SIGQUIT
    exec celery -A treeherder worker --without-gossip --without-mingle --without-heartbeat -Q default,generate_perf_alerts,pushlog,statsd --concurrency=3
...
# Cron jobs
    ...
    ./manage.py update_bugscache

elif [ "$1" == "update_files_bugzilla_map" ]; then
    ./manage.py update_files_bugzilla_map

elif [ "$1" == "update_bugzilla_security_groups" ]; then
    ./manage.py update_bugzilla_security_groups

elif [ "$1" == "cache_failure_history" ]; then
    ./manage.py cache_failure_history
    ...
    shift
    ./manage.py perf_sheriff "$@"

elif [ "$1" == "report_backfill_outcome" ]; then
    shift
    ./manage.py report_backfill_outcome

elif [ "$1" == "update_changelog" ]; then
    ./manage.py update_changelog --days 2
