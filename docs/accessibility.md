...
As a general rule, accessible software is achieved by following web development best practices. As Treeherder uses the [react-bootstrap](https://react-bootstrap.github.io/) library, it is recommended to use one of its components, if possible.
...
If you can build your layout neither with react-bootstrap nor with semantic tags, please insert `aria-role` in them. There are examples of how to use `aria-role` in: [Tables](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Roles/Table_Role), [Progress Bar](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/ARIA_Techniques/Using_the_progressbar_role), [List](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Roles/List_role).
...
Treeherder uses [Bootstrap color](https://getbootstrap.com/docs/5.3/utilities/colors/) utility classes. A few colors were modified in order to meet [WCAG standards](https://developer.mozilla.org/en-US/docs/Web/Accessibility/Understanding_WCAG/Perceivable/Color_contrast?utm_source=devtools&utm_medium=a11y-panel-checks-color-contrast) for color contrast.
...
In some react-bootstrap components, you can insert a `prop` specifying its color. In that case, you use only the color name, without its prefix.
...
```jsx
<Button variant="darker-info">
  Info colored button with contrast passing score
</Button>
```

Known react-bootstrap components that accept the `variant` prop and work with custom Treeherder colors: `Badge`, `Button`, `Card`, `Dropdown`, `Navbar`, `ProgressBar`, `Spinner`.
...
When creating elements that have event listeners, prefer any component of the react-bootstrap interactive elements. Examples are: `Button`, `Form.Control`, `Dropdown`. You can also choose a HTML `<a>` element.
...
There is a special case when you are creating a dropdown menu. First of all, try to follow [react-bootstrap's Dropdown structure](https://react-bootstrap.github.io/docs/components/dropdowns/).

Lastly, insert an additional `as` prop to `Dropdown.Item` component if needed.

```jsx
<Dropdown.Item as="a"> Menu Item </Dropdown.Item>
```

## Forms, inputs and buttons

Apart from inserting high color contrast combinations in buttons, also make sure it has a `:focus` style (usually a blue outline is enough). If you use react-bootstrap's `Button` component, this is added by default, hence it is recommended.
...
For input and checkbox, those are important to be labeled - placeholders are not enough for screen reader users. For example, when inserting an email input and a checkbox, using react-bootstrap, the structure should be:
...
If in doubt, please check the [react-bootstrap Form documentation](https://react-bootstrap.github.io/docs/forms/overview).

## Specific elements

### Progress Bar

Across Perfherder, there are some Progress Bars, which are built using react-bootstrap components. It is recommended to add an `aria-label` to the wrapping component, explaining the values of the bar.
...
>
  <ProgressBar now={valueOne} />
  <ProgressBar now={valueTwo} />
</ProgressBar>
```
...
Also, if you are not using `<table>` tag or react-bootstrap's `Table` component to create the table, please use [all the necessary `role` attributes](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Roles/Table_Role).
