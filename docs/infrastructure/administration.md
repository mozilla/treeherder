...
One of the ways in which we allow users to [access Treeherder data] is via direct access
to our read-only RDS MySQL replica. Both ActiveData and Mozilla's ReDash instance use
this approach.

[access treeherder data]: ../accessing_data.md

Each user should be given a unique MySQL username, created like so:

```sql
CREATE USER 'myuser' IDENTIFIED BY 'PASSWORD THAT IS AT LEAST 30 CHARACTERS LONG' REQUIRE SSL;
...
When tables are added/removed, an updated set of grant statements can be generated using
the following SQL:
...
For new tables the appropriate `GRANT SELECT` statement will need to be manually run for any existing
read-only accounts (this is particularly important for the `activedata` and `redash` accounts).

### Changing MySQL passwords

To change the password for a MySQL account on an RDS instance:

1. [Connect to the instance](#connecting-to-rds-instances) using the existing password.
2. Set the new password [according to the MySQL documentation], ie:

   ```sql
   SET PASSWORD = 'NEW PASSWORD THAT IS AT LEAST 30 CHARACTERS LONG';
   ```

3. [Update the app's environment variable] `DATABASE_URL` to use the new password.

<!-- prettier-ignore -->
!!! note
    Whilst the RDS "master account" password can be changed via the AWS console, this can
    only be performed by an admin of the `moz-devservices` AWS account, so it's easier to
    change the password using MySQL commands.
