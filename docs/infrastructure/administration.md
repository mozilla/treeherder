...
is via direct access to our read-only PostgreSQL stage replica (only a restricted group of performance test team members and Treeherder maintainers should have access to the prototype database). Mozilla's
ReDash instance uses this approach. Only cloudOps can grant access.
...
CREATE USER myuser WITH PASSWORD 'PASSWORD >=30 CHARACTERS LONG';
GRANT SELECT ON ALL TABLES IN SCHEMA public TO myuser;
```

Afterwards provide the user with the newly created credentials and the hostname of the
read-only replica (`psql://<name>:<password>@<replica_host>/treeherder`), making sure
...
To change the password for a PostgreSQL account (only cloudOps can do this):

1. Set the new password using the `ALTER ROLE` command.

   For example to change another user's password:

   ```sql
   ALTER ROLE another_user WITH PASSWORD 'NEW PASSWORD AT LEAST 30 CHARACTERS LONG';
   ```
