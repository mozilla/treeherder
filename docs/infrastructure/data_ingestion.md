...
Treeherder executes the `pulse_listener_pushes`, `pulse_listener_tasks` and `pulse_listener_tasks_classification` django commands in [entrypoint_prod](https://github.com/mozilla/treeherder/blob/master/docker/entrypoint_prod.sh) that listen to both the main firefox-ci cluster and the community cluster. They add tasks to the `store_pulse_pushes`, `store_pulse_tasks` and `store_pulse_tasks_classification` queues for `worker_store_pulse_data` to process. The user credentials (treeherder-prod, treeherder-staging and treeherder-prototype) are stored in the `PULSE_URL` env variable; `PULSE_PUSH_SOURCES`, `PULSE_TASK_SOURCES` and `PULSE_MOZCI_CLASSIFICATION_SOURCES` can override the default list of clusters and credentials. Local development runs the combined `pulse_listener` command instead, configured by `PULSE_SOURCES`. All listeners honor `SKIP_INGESTION=True`.
