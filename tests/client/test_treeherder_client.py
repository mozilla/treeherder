...
class TreeherderClientTest(DataSetup, unittest.TestCase):
    ...
    def test_get_job(self, mock_get):
        ...
        self.assertEqual(jobs, [{"jobDetail1": 1},
                                {"jobDetail2": 2},
                                {"jobDetail3": 3}
                                ])

    @patch("treeherder.client.client.requests.get")
    def test_get_results(self, mocke_get):

        mocke_get.return_value = self._get_mock_response({
            "meta": {"count": 3, "repository": "mozilla-inbound",
                     "offset": 0},
            "results": [{"resultSet1": 1},
                        {"resultSet2": 2},
                        {"resultSet3": 3}
                        ]})
        ...
        self.assertEqual(resultSets, [{"resultSet1": 1},
                                      {"resultSet2": 2},
                                      {"resultSet3": 3}
                                      ])
