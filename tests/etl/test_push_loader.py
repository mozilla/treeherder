...
def test_ingest_github_push_bad_repo(github_push):
    ...
    github_push[1]["payload"]["details"]["event.head.repo.url"] = "https://bad.repo.com"
    PushLoader().process(github_push[1]["payload"], "exchange/taskcluster-github/v1/push", "https://tc.example.com")
    assert Push.objects.count() == 0
...
def test_ingest_github_push_comma_separated_branches(branch, expected_pushes, github_push,
    ...
    github_push[1]["payload"]["details"]["event.base.repo.branch"] = branch
    assert Push.objects.count() == 0
    PushLoader().process(github_push[1]["payload"], "exchange/taskcluster-github/v1/push", "https://tc.example.com")
