...
def test_performance_signatures_are_deleted(test_perf_signature, monkeypatch):
    monkeypatch.setattr(taskcluster, 'Notify', MagicMock())
...
def test_signature_remover(test_perf_signature, test_perf_signature_2, test_perf_data, monkeypatch):
    monkeypatch.setattr(taskcluster, 'Notify', MagicMock())
    cycler = PerfherderCycler(chunk_size=100, sleep_time=0)
    expired_timestamp = cycler.max_timestamp
...
def test_signature_remover_when_notify_service_is_down(
    test_perf_signature, test_perf_signature_2, test_perf_data, monkeypatch
):
    monkeypatch.setattr(taskcluster, 'Notify', MagicMock())
...
@pytest.mark.parametrize('signatures_nr', [3, 4, 8, 10])
def test_nr_emails_sent(test_perf_signature, signatures_nr):
    tc_model = MagicMock()
    timer = MaxRuntime()
    timer.started_at = datetime.now()
    nr_of_rows = 2
    nr_of_emails = 4
    signatures_remover = PublicSignaturesRemover(
        timer=timer,
        taskcluster_model=tc_model,
        nr_of_rows_allowed=nr_of_rows,
        nr_of_emails_allowed=nr_of_emails,
    )

    for nr in range(0, signatures_nr):
        PerformanceSignature.objects.create(
            repository=test_perf_signature.repository,
            signature_hash=(20 * ('t%s' % nr)),
            framework=test_perf_signature.framework,
            platform=test_perf_signature.platform,
            option_collection=test_perf_signature.option_collection,
            suite='mysuite%s' % nr,
    ...
    signatures_nr += 1  # is incremented because of test_perf_signature
    nr_of_possible_emails = math.ceil(signatures_nr / nr_of_rows)
    expected_call_count = (
        nr_of_possible_emails if nr_of_possible_emails <= nr_of_emails else nr_of_emails
    )

    signatures_remover.remove_in_chunks(datetime.now())
