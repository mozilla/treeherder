...
def parse_build_platform(name, os_field="os_name"):
    ...
    architectures = ["x86_64", "x86", "aarch64", "armv7", "arm"]
    platform = [p for p in platforms if p in name.lower()]
    if not platform:
        return {}
    os_name = platform[0]
    arch = [a for a in architectures if a in name]
    if not arch:
        # look for 64 or 32:
        if "32" in name:
            arch = "x86"
            platform = f"{os_name}32"
        elif "64" in name:
            arch = "x86_64"
            platform = f"{os_name}64"
        else:
            return {}
    else:
        arch = arch[0]
    ...
    for task in all_tasks:
        ...
        results = {
            ...
            "job": {
                ...
                "name": task["job_group_name"].split(" ")[0],
...
def get_push_data(project, output_file):
    ...
    # ideally autoland `hg log -50`, then pick #50 and and #45
...
def get_job_data(project, revisions, output_file):
    ...
    while total_tasks < 100:
        ...
        jobdata = tasks_to_jobdata(all_tasks, project, rev)
        total_tasks += len(jobdata)
        print(f"  found {len(jobdata)} tasks to add to job data!")
        jobdata.extend(jobdata)
        revision_index -= 5
...
def generate_pulse_job_data(tasks, revision, output_file):
    ...
    for task in tasks:
        ...
        blob = {
            ...
            "origin": {"kind": "hg.mozilla.org", "project": "set by test", "revision": revision},
            ...
            "jobInfo": {
                "links": [
                    {"url": log["url"], "linkText": log["name"], "label": "artifact uploaded"}
