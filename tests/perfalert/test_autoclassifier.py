...
def test_get_resultset_from_hg():
    """Verifies that communication to hg works as intended"""

    get_result_set_from_hg("https://hg.mozilla.org/integration/autoland", "c2463be000652c901f160910b4075d081b6c664f",
                           "3bf7045fe191b88e0b5ba1eadaf6d6824610a00c")


def test_is_downstream_resultset():
    """Verifies that known downstream resultsets are correctly identified"""
    assert is_downstream_resultset("https://hg.mozilla.org/integration/autoland",
                                   "30f0eae664b4ef1e328d6b46c658e811e9edef4e",
                                   "3a5db9f20b43e149a61507707b559773e6a3debe") == True

    assert is_downstream_resultset("https://hg.mozilla.org/integration/mozilla-inbound",
                                   "a213fcd04400dd5db3514693d61a1c93f8478c76",
                                   "00195e9e7d8b954681a8c2a0784442c33e690684") == True

    assert is_downstream_resultset("https://hg.mozilla.org/integration/mozilla-inbound",
                                   "f5fe9e219441413c6fc8429050ca19c910b7c62a",
                                   "da8d6c4eab6142c98b04706255498750076fee9d") == False
...
def test_get_possible_upstreams(test_repository, test_perf_signature,
    ...
    assert get_possible_upstreams(downstreams[0], upstreams).count() == 1
    assert get_possible_upstreams(downstreams[1], upstreams).count() == 1

    # TODO
    # need to test with different signatures
