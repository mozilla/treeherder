...
def test_publish_stats_windowing(
    ...
    _create_job("guid_before", push_before, generic_reference_data, end_time=before_time)
    _create_job("guid_inside", push_inside, generic_reference_data, end_time=inside_time)
...
@pytest.mark.django_db
@patch("treeherder.workers.stats.get_stats_client")
def test_publish_stats_pushes_only(get_worker_mock, create_push, test_repository, caplog, settings):
    settings.CELERY_STATS_PUBLICATION_DELAY = 10
    statsd_client = MagicMock()
    get_worker_mock.return_value = statsd_client
...
def test_publish_stats_jobs_only(
    ...
    push_before = create_push(test_repository, revision="rev_before", time=before_time)
    _create_job("guid_job_only", push_before, generic_reference_data, end_time=inside_time)
