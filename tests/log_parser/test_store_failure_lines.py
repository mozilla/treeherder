...
# TODO: JMAHER: this store_failure_lines causes problems with other tests (currently)
...
def mock_log_parser(job_logs, mock_parser):
    from treeherder.log_parser.tasks import store_failure_lines
    ...
    try:
        ...
        for jl in job_logs:
            ...
            # NOTE: do this twice to ensure we pick everything up
    #            store_failure_lines(jl)
    except:
        raise
        ...
        job_def["job"].update(
            {
                ...
                "job_guid": job_def["job"]["job_guid"].replace("e", str(random.randint(0, 9))),
                "start_timestamp": job_def["job"]["start_timestamp"] + 100 + random.randint(0, 100),
...
def test_infra_no_intermittent(activate_responses, hundred_job_blobs, mock_parser, create_jobs):
    ...
    job_log_ids = [j.id for j in job_logs]

    # this will parse and check for intermittents
    mock_log_parser(job_logs, mock_parser)

    # (4 == intermittent, 1 == not classified)
    j1 = Job.objects.filter(id=jobs[0].id)
    assert j1[0].failure_classification.id == 1

    j2 = Job.objects.filter(id=jobs[1].id)
    assert j2[0].failure_classification.id == 1
...
def test_infra_intermittent(activate_responses, hundred_job_blobs, mock_parser, create_jobs):
    ...
    # (4 == intermittent, 1 == not classified)
    # TODO: this should be intermittent, but we need to solve this another way
    j1 = Job.objects.filter(id=jobs[0].id)
    assert j1[0].failure_classification.id == 4
...
def test_multiple_jobs_intermittent(
    ...
    j2 = Job.objects.filter(id=jobs[1].id)
    assert j2[0].failure_classification.id == 4
...
def test_confirm_failure_no_intermittent(
    ...
    job_logs = JobLog.objects.filter(job_id__in=(j.id for j in jobs))
    print(
        f"JMAHER: 3 filenames, jobs: {len(jobs)}, logs: {len(job_logs)}, jobids: {(j.id for j in jobs)}"
    )
...
def test_confirm_failure_pass_intermittent(
    ...
    job_logs = JobLog.objects.filter(job_id__in=(j.id for j in jobs))
    print(
        f"JMAHER: 4 filenames, jobs: {len(jobs)}, logs: {len(job_logs)}, jobids: {(j.id for j in jobs)}"
    )
