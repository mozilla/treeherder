...
describe('Taskcluster actions', () => {
  ...
  const tcActionsUrl = `${rootUrl}/api/queue/v1/task/LVTawdmFR2-uJiWWS2NxSw/artifacts/public/actions.json`;
  ...
  const buildTaskAction = (inputName) => {
    return {
      name: inputName,
      kind: 'task',
      context: [{ test: 'test' }],
  ...
  beforeEach(() => {
    ...
    fetchMock.get(tcActionsUrl, {
      version: 1,
      actions: [buildTaskAction('foo'), buildTaskAction('cancel')],
    });
  ...
  test('cancelAll uses passed-in decisionTask', async () => {
    ...
    await JobModel.cancelAll(526443, currentRepo, () => {}, decisionTask, true);
