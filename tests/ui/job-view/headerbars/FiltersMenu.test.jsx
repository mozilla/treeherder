...
describe('FiltersMenu', () => {
  ...
  it('calls pinJobs when "Pin all showing" is clicked', () => {
    ...
    expect(pinnedJobsActions.pinJobs).toHaveBeenCalledWith([
      { id: 1, jobType: 'test' },
      { id: 2, jobType: 'build' },
    ]);
  });
  ...
  it('calls setSelectedJob when pinning jobs and no job is selected', () => {
    ...
    expect(selectedJobActions.setSelectedJob).toHaveBeenCalledWith({
      id: 1,
      jobType: 'test',
    });
  });
  ...
  it('creates correct URL for "My pushes only" link', () => {
    ...
    expect(myPushesLink.search).toContain('author=test@example.com');
  });
