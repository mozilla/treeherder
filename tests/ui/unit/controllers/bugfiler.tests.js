...
describe('BugFilerCtrl', function(){
    ...
    beforeEach(inject(function ($injector, $rootScope, $controller) {
        ...
        bugFilerScope.productSearch = "";
        bugFilerScope.selection={}
    ...
    /*
        Tests BugFilerCtrl
     */
    it('should find the product by metadata', function(done) {
        $httpBackend.expect('GET', 'https://hg.mozilla.org/mozilla-central/json-mozbuildinfo?p=browser/components/search/test/browser_searchbar_smallpanel_keyboard_navigation.js').respond({
          "aggregate": {
        ...
        bugFilerScope.findProduct().then(function(blah) {
           expect(bugFilerScope.suggestedProducts).toEqual([ 'Firefox :: Search' ]);
           expect(bugFilerScope.selection.selectedProduct).toBe("Firefox :: Search");
           done();
        }, function(blah) {
           done();
        });
    ...
    it('should find the talos product by missing metadata', function(done) {
        bugFilerScope.selectedJob.job_group_name = "Talos Performance e10s";
        bugFilerScope.parsedSummary = [["a11yr", "application crashed [unknown top frame]"], "a11yr"]
        $httpBackend.expect('GET', 'https://hg.mozilla.org/mozilla-central/json-mozbuildinfo?p=a11yr').respond({
            "aggregate": {
              "bug_component_counts": [], 
        ...
        bugFilerScope.findProduct().then(function(blah) {
            expect(bugFilerScope.suggestedProducts).toEqual([ 'Testing :: Talos' ]);
            expect(bugFilerScope.selection.selectedProduct).toBe("Testing :: Talos");
            done();
        }, function(blah) {
            done();
        });
    ...
    it('should find the product by bugzilla', function(done) {
        $httpBackend.expect('GET', 'https://bugzilla.mozilla.org/rest/prod_comp_search/Firefox :: Search?limit=5').respond({
            "products":[
              {
                "product":"Firefox"
              },
        ...
        bugFilerScope.productSearch = "Firefox :: Search";
        bugFilerScope.findProduct().then(function(blah) {
           expect(bugFilerScope.suggestedProducts).toEqual([ 'Firefox :: Search', 'Marketplace :: Search', 'Firefox for Android :: Search Activity', 'Firefox OS :: Gaia::Search', 'Cloud Services :: Operations: Storage' ]);
           expect(bugFilerScope.selection.selectedProduct).toBe("Firefox :: Search");
           done();
        }, function(blah) {
           done();
    ...
    it('should parse summaries', function() {
        ...
        // Test parsing xpcshell tests
        summary = "xpcshell-child-process.ini:dom/indexedDB/test/unit/test_create_index_with_integer_keys.js | application crashed [@ mozalloc_abort(char const*)]";
        summary = bugFilerScope.parseSummary(summary);
        expect(summary[0][0]).toBe("dom/indexedDB/test/unit/test_create_index_with_integer_keys.js");
        expect(summary[0][1]).toBe("application crashed [@ mozalloc_abort(char const*)]");
        expect(summary[1]).toBe("test_create_index_with_integer_keys.js");

        // Test stripping omitted leads
        summary = "REFTEST TEST-UNEXPECTED-FAIL | file:///home/worker/workspace/build/tests/reftest/tests/layout/reftests/mathml/mfrac-A-8.html == file:///home/worker/workspace/build/tests/reftest/tests/layout/reftests/mathml/mfrac-A-8-ref.html | image comparison, max difference: 255, number of differing pixels: 1000";
        summary = bugFilerScope.parseSummary(summary);
        expect(summary[0][0]).toBe("layout/reftests/mathml/mfrac-A-8.html == layout/reftests/mathml/mfrac-A-8-ref.html");
        expect(summary[0][1]).toBe("image comparison, max difference: 255, number of differing pixels: 1000");
        expect(summary[1]).toBe("mfrac-A-8.html");
    });
