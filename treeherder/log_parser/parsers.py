...
class ErrorParser(ParserBase):
    ...
    # We require a space inside the square brackets because lines beginning
    # with "[taskcluster:error]" signify an error and must not be ignored.
    RE_TASKCLUSTER_PREFIX = re.compile(r"^\[[^\]]+\s[^\]]+\]\s")
