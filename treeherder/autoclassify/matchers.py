...
from treeherder.model.models import (MatcherManager,
                                     TextLogError,
                                     TextLogErrorMatch)
from treeherder.services.elasticsearch import es_conn
...
class ElasticSearchTestMatcher(Matcher):
    ...
    def query_best(self, text_log_error):
        ...
        try:
            self.calls += 1
            results = es_conn.search(query, index=INDEX_NAME)
        except Exception:
        ...
        matches = [(item, item.message) for item in results['hits']['hits']]
