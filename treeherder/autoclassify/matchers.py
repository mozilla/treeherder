...
ignored_line = (Q(text_log_error__best_classification=None) &
                Q(text_log_error__best_is_verified=True))
...
class PreciseTestMatcher(Matcher):
    ...
    @id_window(size=20000,
               time_budget=500)
    def query_best(self, text_log_error):
        failure_line = text_log_error.failure_line
        ...
        return [(TextLogErrorMatch.objects
                 .filter(text_log_error__failure_line__action="test_result",
                         text_log_error__failure_line__test=failure_line.test,
                         text_log_error__failure_line__subtest=failure_line.subtest,
                         text_log_error__failure_line__status=failure_line.status,
                         text_log_error__failure_line__expected=failure_line.expected,
                         text_log_error__failure_line__message=failure_line.message)
...
class CrashSignatureMatcher(Matcher):
    ...
    def query_best(self, text_log_error):
        ...
        matching_failures = (TextLogErrorMatch.objects
                             .filter(text_log_error__failure_line__action="crash",
                                     text_log_error__failure_line__signature=failure_line.signature)
                             .exclude(ignored_line |
                                      Q(text_log_error__step__job=text_log_error.step.job))
                             .select_related('text_log_error')
                             .order_by("-score", "-classified_failure"))

        return [matching_failures.filter(text_log_error__failure_line__test=failure_line.test),
