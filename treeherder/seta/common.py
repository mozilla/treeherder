...
def job_priority_index(job_priorities):
    ...
    jp_index = {}
    # Creating this data structure which reduces how many times we
    # iterate through the DB rows
    for jp in job_priorities:
        key = jp.unique_identifier()
        if key in jp_index:
            # This is guaranteed by a unique composite index for
            # these 3 fields in models.py
            raise ValueError(
                '"{}" should be a unique job priority and '
                'that is unexpected.'.format(key)
            )
