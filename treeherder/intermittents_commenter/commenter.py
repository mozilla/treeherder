...
from treeherder.model.models import BugJobMap, Bugscache, OptionCollection, Push
...
class Commenter:
    ...
    def get_all_test_variants(self, testrun_os_matrix, platform, os_name, architecture):
        ...
        if version in testrun_os_matrix:
            variants = set(testrun_os_matrix[version][architecture].keys())
        else:  # win and android (we tke the first availlable version)
            variants = set(list(testrun_os_matrix.values())[0][architecture].keys())
