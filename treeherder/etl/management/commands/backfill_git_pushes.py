...
class Command(BaseCommand):
    ...
    def _report_results(self, repo, stats, options):
        ...
        if options["flip_dvcs_type"]:
            branch_name = repo.git_branch or "main"
            repo.dvcs_type = "git"
            repo.url = repo.git_url
            repo.save()
            # Repository no longer has a "branch" field directly; branches are
            # tracked via the RepositoryBranch model (see model.models).
            RepositoryBranch.objects.get_or_create(repository=repo, branch=branch_name)
            self.stdout.write(
                f"  Flipped {repo.name} to dvcs_type='git', "
                f"url='{repo.git_url}', branch='{branch_name}'"
            )
