...
class FailureMatch(models.Model):
    id = BigAutoField(primary_key=True)
    failures = models.ManyToManyField(Failure, through='FailureMatchFailure',
...
class FailureMatchFailure(models.Model):
    id = BigAutoField(primary_key=True)
    failure = FlexibleForeignKey(Failure)
    failure_match = FlexibleForeignKey(FailureMatch)
