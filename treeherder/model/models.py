...
class FailureLine(models.Model):
    ...
    def __str__(self):
        return "id:%s job:%s" % (self.id, Job.objects.get(guid=self.job_guid).id)
...
class ClassifiedFailure(models.Model):
    ...
    def __str__(self):
        return "id:%s bug_number:%s" % (self.id, self.bug_number)
...
class FailureMatch(models.Model):
    ...
    def __str__(self):
        return "failure_line:%s classified_failure:%s" % (
...
class TextLogError(models.Model):
    ...
    def __str__(self):
        return "id:%s job:%s" % (self.id, self.step.job.id)
...
class TextLogErrorMatch(models.Model):
    ...
    def __str__(self):
        return "text_log_error:%s classified_failure:%s" % (
