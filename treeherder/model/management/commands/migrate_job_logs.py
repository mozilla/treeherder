            ...
            while True:
                ...
                if job_ids:
                    job_id_mapping = dict((project_specific_id, job_id) for
                                          ...
                                          job_id_pairs)
                    c.execute("""SELECT job_id, name, url, parse_status from job_log_url where job_id>{} limit 0,10000""".format(offset))
                    job_logs = []
                    ...
                    self.stdout.flush()
                offset = max([job_id_pair[1] for job_id_pair in job_id_pairs])
                self.stdout.write("\n")
