...
class JobsModel(TreeherderModelBase):
    ...
    def get_possible_job_list(self):
        data = self.execute(
            proc="jobs.selects.get_possible_job_list",
            replace=[self.refdata_model.get_db_name()],
            debug_show=self.DEBUG,
        )

        # These jobs must show up in Treeherder as possible jobs
        for datum in data:
            datum['job_coalesced_to_guid'] = None
            datum['state'] = 'possible'
            datum['result'] = 'possible'
...
into chunks of chunk_size size. Returns the number of result sets deleted"""
    ...
    def clean_possible_job_data(self):
        """
        Remove possible jobs from the previous day.

        This way we always have an up-to-date possible jobs lists.
        """
        self.execute(
            proc='jobs.deletes.cycle_possible_job',
            debug_show=self.DEBUG)
