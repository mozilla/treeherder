...
class TreeherderClient(object):
    ...
    def query_resultset_api(self, project, **filter_params):
        """
        Query the /resultset/ api of treeherder

        :param project: project (repository name) to query data for
        :param filter_params: keyword arguments to filter results
        """

        filter_string = '&'.join("%s=%s" % (key, val) for key, val in filter_params.iteritems())
        response = self._get_json(project, self.RESULTSET_ENDPOINT.format(filter_string))
        return response["results"]

    def query_jobs_api(self, project, **filter_params):
        """
        Query the /jobs/ api of treeherder
        ...
        filter_string = '&'.join("%s=%s" % (key, val) for key, val in filter_params.iteritems())
        response = self._get_json(project, self.JOBS_ENDPOINT.format(filter_string))
        return response["results"]
