        ...
        ThRepositoryModel.load().then(function () {
            ...
            $scope.addTestData = function (option, seriesSignature) {
                if ($scope.seriesList.length > 0) {
                    ...
                    $scope.defaultProjectName = lastSeries.projectName;
                    $scope.defaultPlatform = lastSeries.platform;
                    $scope.defaultFrameworkId = lastSeries.frameworkId;
                }

                if (option !== undefined) {
                    var series = _.find($scope.seriesList, { signature: seriesSignature });
                    $scope.options = { option: option, relatedSeries: series };
                }
