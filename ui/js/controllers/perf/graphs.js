...
perf.controller('GraphsCtrl', [
    ...
    function GraphsCtrl($state, $stateParams, $scope, $rootScope, $location,
        $window, thServiceDomain, $http, $q, $timeout, PhSeries,
        ...
        ThRepositoryModel.load().then(function () {
            ...
            $scope.graphsDialogInit = function (option, seriesSignature) {
                let defaultProjectName, defaultPlatform, defaultFrameworkId;
                let options = {};
                if ($scope.seriesList.length > 0) {
                    const lastSeries = $scope.seriesList.slice(-1)[0];
                ...
                if (option !== undefined) {
                    const series = _.find($scope.seriesList, { signature: seriesSignature });
                    options = { option: option, relatedSeries: series };
                }

                return {
                    projects: $rootScope.repos,
                    timeRange: $scope.myTimerange.value,
                    testsDisplayed: $scope.seriesList,
                    defaultFrameworkId,
                    defaultProjectName,
                    defaultPlatform,
                    options
            ...
            $scope.graphsDialogSuccessCallback = function (seriesList) {
                $scope.loadingGraphs = true;
