...
perf.value('compareDefaultTimeRange', 86400 * 2);

perf.factory('compareFactory',[
    '$rootScope', 'ThResultSetModel',
    function ($rootScope, ThResultSetModel) {
        return {
            verifyRevision : function(project, revision, rsid){
                return ThResultSetModel.getResultSetsFromRevision(project.name, revision).then(
                    function(resultSets) {
                        ...
                        if (rsid === 'original') {
                            $rootScope.originalResultSet = resultSet;
                        } else {
                            $rootScope.newResultSet = resultSet;
                        }
                    },
                    function(error) {
                        $rotScope.errors.push(error);
                    });
            }};
    }
]);
    ...
    function CompareChooserCtrl($state, $stateParams, $scope, $q,
        ...
        ThRepositoryModel.get_list().success(function(projects) {
            ...
            $scope.runCompare = function() {
                var orignalRevisionPromise;
                if($scope.originalRevision.length){
                    orignalRevisionPromise =  ThResultSetModel.getResultSetsFromRevision($scope.originalProject.name, $scope.originalRevision).then(
                    function(resultSets) {
                ...
                var newRevisionPromise =  ThResultSetModel.getResultSetsFromRevision($scope.newProject.name, $scope.newRevision).then(
                    function (resultSets) {
                ...
                $q.all([orignalRevisionPromise, newRevisionPromise]).then(function(){
...
perf.controller('CompareResultsCtrl', [
    ...
    function CompareResultsCtrl($state, $stateParams, $scope,
        ...
        function load() {
            ...
            if ($scope.originalRevision.length){
                // ShrutiJ: originalResulSet can't be defined for baseline comparison, requires revision
                ...
                var timeRange = phTimeRanges;
                var selectedTimeRange = _.find($scope.timeRanges, {
                    value: ($stateParams.selectedTimeRange) ? parseInt($stateParams.selectedTimeRange) : compareDefaultTimeRange
                });
                var originalSeriesListseriesList;

                originalSeriesList = PhSeries.getSeriesList($scope.selectedRepo.name, {
        ...
        if($stateParams.originalRevision !== undefined) {
            $q.all([loadRepositories, loadFrameworks]).then(function() {
                ...
                compareFactory.verifyRevision($scope.originalProject, $scope.originalRevision, "original").then(function () {
                    compareFactory.verifyRevision($scope.newProject, $scope.newRevision, "new").then(function () {
                        if ($scope.errors.length > 0) {
                            $scope.dataLoading = false;
                            return;
                        }
            ...
            //ShrutiJ: A seperate function verifyBaseline like verifyRevision, can be defined in factory
            console.log("Define Functions for baseline comparison");
        }
    }]);
