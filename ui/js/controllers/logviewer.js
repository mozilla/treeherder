...
logViewerApp.controller('LogviewerCtrl', [
    ...
    'dateFilter', 'thJobSearchStr', 'thResultStatus', 'ThResultSetModel',
    function Logviewer(
        $anchorScroll, $http, $location, $q, $rootScope, $scope,
        $timeout, ThJobArtifactModel, ThLog, ThLogSliceModel, ThJobModel,
        dateFilter, thJobSearchStr, thResultStatus, ThResultSetModel) {
        ...
        $scope.init = function() {
            ...
            ThJobModel.get($scope.repoName, $scope.job_id).then(function(job) {
                ...
                $scope.logProperties.push({label: "Job", value: jobStr});

                $scope.logProperties.push({label: "Start", value: dateFilter(job.start_timestamp*1000, 'MMM d, y h:mm:ss a Z')});
                $scope.logProperties.push({label: "End", value: dateFilter(job.end_timestamp*1000, 'MMM d, y h:mm:ss a Z')});
                $scope.logProperties.push({label: "Machine", value: job.machine_name});

                $scope.result = {label: "Result", value: job.result};
                $scope.resultStatusShading = $scope.getShadingClass(job.result);

                ThResultSetModel.get($scope.repoName, job.result_set_id).then(function(data){
