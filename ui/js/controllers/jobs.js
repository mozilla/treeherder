...
treeherderApp.controller('ResultSetCtrl', [
    ...
    function ResultSetCtrl(
        ...
        $scope.triggerAllTalosJobs = function(revision) {
            ...
            var times = window.prompt("Enter number of instances to have for all talos jobs", 6);
            while (times > 6) {
                times = window.prompt("We do not allow instances of all talos jobs to be greater than 6 times. Enter again", 6);
            }

            for (var iter=1; iter <= times; iter++) {
                ThResultSetModel.triggerAllTalosJobs($scope.resultset.id, $scope.repoName).then(function() {
                    thNotify.send("Request sent to trigger all talos jobs " + iter + " time", "success");
                }, function(e) {
                    thNotify.send(
                        ThModelErrors.format(e, "The request for 'trigger all talos jobs' failed"),
                        'danger', true
                    );
                });
            }
