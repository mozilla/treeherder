...
treeherder.controller('JobsCtrl',
    function JobsCtrl($scope, $http, $rootScope, $routeParams, $log,
        ...
        $scope.rsUpdateQueue = [];
        var updateQueueProcessInterval = 60000; // once per minute

        $scope.rsMap = {};
        // straight job to job_id mapping
        $scope.jobMap = {};
        $scope.jobMapOldestId = null;
        $scope.rsMapOldestId = null;

        /******
         * Get a list of resultsets and add it to the data model.
         * If ``resultsetlist`` is included, then it prepends them.  If not,
         * they are appended.
         *
         * @param count Number of items to fetch.  If ``resultsetlist`` is
         *              provided, this gets set to the size of that list.
         * @param resultsetlist Array of resultset ids to fetch.
         */
        $scope.fetchResultSets = function(count, resultsetlist) {
            ...
            thResultSets.getResultSets($scope.offset, count, resultsetlist).
                success(function(data) {
                    $scope.offset += data.length;
                    if (resultsetlist) {
                        // if we specify a ``resultsetlist`` then we want to
                        // prepend the resultsets because they'll be newer.
                        $scope.result_sets.unshift($scope.result_sets, data);
