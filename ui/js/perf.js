...
perf.service('JsonPushes', ['$http','$q', function($http, $q) {
    ...
    var prevRevsErrHandler = function(response) {
        if (response instanceof Error) {
            return $q.reject("Unable to get previous revisions" +
                             " (" + response + ").");
        } else {
            // http error
    ...
    var _getPreviousRevisionFrom = function(project, revision, projects,
                                             defer, attempt, max_attempts) {
        attempt +=1;
        if (attempt >= max_attempts) {
            defer.reject("Unable to find a previous revision");
            return;
        }
                ...
                return $q(function(resolve, reject) {
                    $http.get(
                        ...
                        function(data) {
                            ...
                            // swallow all errors so $q.all will success
                            resolve(null);
                        }
                    );
                });
