...
treeherder.component('phCompareTable', {
  ...
  controller: function() {
    ...
    ctrl.getCompareClasses = function(cr, type) {
      ...
      if (type === 'row' && cr.highlightedTest)
    ...
    function filter(item, matchText) {
      return (
        !matchText || item.toLowerCase().indexOf(matchText.toLowerCase()) > -1
      );
    }
    function shouldBeShown(result) {
      return (
        (!ctrl.filterOptions.showOnlyImportant || result.isMeaningful) &&
    ...
    function filterResult(results, key) {
      ...
      return results.filter(result => {
        ...
        return ctrl.filterOptions.filter
          .split(' ')
          .every(
            matchText =>
    ...
    function() {
      ...
      ctrl.$onInit = function() {
        ...
        ctrl.replicates.forEach(value => {
          ctx.beginPath();
          ctx.arc(
            (180 / (ctrl.maxValue - ctrl.minValue)) * (value - ctrl.minValue) + 5,
            18,
            5,
            0,
            360,
