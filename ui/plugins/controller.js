                            ...
                            $q.all(_.chunk(signatureList, 20).map(function(signatureChunk) {
                                ...
                                _.forEach(seriesList, function(series) {
                                    ...
                                    detail['url'] = generateUrlForPerf($scope.repoName, $scope.job['id'],
                                        series.signature, $scope.job['result_set_id']);
                                    detail['value'] = $scope.job_ids[series.signature][0].value;
                                    detail['suite'] = series.suite;
                                    if (series.hasOwnProperty('test')) {
                                        detail['suite'] = series.suite + '_' +
                                            series.test.toLowerCase();
                                    }
